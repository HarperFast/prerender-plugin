	...
	if (finalRes) {
		...
		if (statusCode === 200) {
			...
			if (job.isIndexable || job.isFromSitemap) {
				...
				//
				// The page's own SEO facts ride the same evaluate, under exactly the same contract: one CDP
				// round trip and one JSON-LD parse for both. Each reader is caught inside the page, so a
				// throw in one still costs the consumer only that claim; a failure of the evaluate itself
				// posts null for both. `?? null` keeps a missing key from ever reading as absent.
