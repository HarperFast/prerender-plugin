...
| `queue_health`   | value   | series     | result      | —          | Every queue signal in one scan: the snapshot gauges (`overdue`, `lease_occupancy`, `below_floor`, `below_floor_age_ms`, `floor_pin_age_ms`, `paused`), `claim_scan_ms` (per pass, method = granted/empty/capped), `lane_granted` (per lane per pass, method = the lane label), `reconcile_restored`/`reconcile_missing` (per sweep). |
...
| `queue_health` `lane_granted` at zero for a lane whose backlog is not empty         | That lane is being starved. Either its `queue.lanes.minShare` floor is too low, or `ttlBands` put two very different cadences in one band so a slower route is consuming it. Lanes move no totals, so nothing else shows this. |
