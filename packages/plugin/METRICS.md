...
| Question                                 | Read this                                                                                                                                                                                                                                                                                                                                                                                       |
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Are we taking load off the origin?       | GROSS: `bot_serve` share of rows with `path != 'origin'`. NET (what the origin actually saw): `1 − (bot_serve origin + render outcome count + Σ render subrequests uncacheable + Σ probe_probed + Σ sitemap_sitemaps + Σ hydration_calls incurred) ÷ (bot_request + Σ hydration_calls saved + Σ hydration_calls incurred)` — every Σ is mean × count. The console's Traffic view computes both. |
...
- **`queue_health`'s snapshot series and `prerender_ops`' `demand_*` are gauges on a slow cadence**, one set per node
  ...
  never sum a gauge over time. `overdue` sums across nodes; `fill` takes its **peak** (below). The
  ladder's decision counters are NOT gauges — they are per-interval counts and must be summed.
- **`demand_fill` is a sawtooth — read max/p95, never the mean.** It is the set-bit fraction of the
  ...
  including at the top. The false-positive rate is `fill^k` (k = `render.demand.hashes`, 7 by
  default), and that exponent is what makes the distinction matter rather than nitpicking: a mean of
  0.75 implies 13% false positives, the 0.986 peak of the same slice implies **91%**. A saturated ring
  reads "visited" for everything, so the ladder promotes the whole corpus to its floor — cadence
...
- **`page_verification` (a `prerender_ops` series) is off by default**, and only ever writes while an
  invalidation is active for a probe rule's `invalidateScope`. No rows means either the feature is
  disabled or nothing is currently invalidated — those two are distinguished by
  `GET /prerender_admin/invalidations`, not by this series.
- **`bot_serve` cacheStatus `verified` is a CACHE SERVE and counts toward origin offload**, but it is
  deliberately NOT folded into `hit`: the page is as old as it ever was, and it is being served on
  evidence (the change probe re-confirmed its claims against the origin after the epoch) rather than
  on age. Read `verified` against `invalidated` to size what per-page verification is buying —
  together they are the population a bulk invalidation touched, split into rescued and refused.
  Folding it into `hit` would inflate the freshness signal with proven-but-old pages, which is the
  same readability failure that made `swr` its own status.
- **`not-owner` is the number to read first when the accelerator looks ineffective**, and it must be
  read against the per-node bot-traffic split, not on its own. Residency is uniform (~1/N per node)
  while traffic lands where the CDN's geo-routing sends it; where those two disagree — measured at
  70/21/6/3 on a four-node deployment — `not-owner` reaches 84-85% and the "crawlers revisit"
  fallback cannot recover it, because the crawler keeps returning to the same busy node.
  `invalidation.reenqueue.crossNode` is what converts those into `forwarded`.
...
- **`hydration_calls` and `render` `subrequests` are the two halves of one number, and both are
  VALUES**: read Σ as mean × count, never `count`. `count` on `hydration_calls` is serves to
  script-executing crawlers (useful for the `unknown` side, where it is the size of the blind spot);
  `count` on `subrequests` is results. k is the render fleet's measurement of the ORIGIN page under
  its own block list: it undercounts by whatever that list aborts (`blocked` is reported beside it)
  and takes no position on responses with no freshness headers (`unspecified` — a CDN's default TTL
  decides those, and this plugin cannot see it). Which crawlers execute scripts is the registry's
  claim (`analytics.bots[].rendersJs`), not an observation — flag a bot only on its vendor's
  documentation. `unknown` decays over one render cycle after the fleet upgrade to browser 1.22.0,
  as pages re-render and acquire a k; a corpus whose serves are mostly `unknown` is one whose net
  offload is still documents-only.
- **Renamed in 0.39.0** (this plugin owns its only metric consumers, so the break was taken
  ...
  `invalidation_error`, `invalidation_reenqueue`, `page_verification`, `page_age_negative` → `prerender_ops` series of
