...
export function backoffWait(interval, strikes, fromSitemap) {
	...
	const wait = interval * backoffFactor ** escalations * penalty;

	// THE TRAP: `maxBackoff` is a ceiling on the BACKOFF, and a ceiling below the target's own
	// cadence would make a persistently failing page come due MORE often than a healthy one (a 48h
	// PDP against a 24h ceiling). Never return less than the normal interval — the floor is the
	// cadence, the ceiling only ever pushes further out.
	return Math.max(interval, Math.min(wait, maxBackoff));
}
